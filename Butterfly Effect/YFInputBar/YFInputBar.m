...
-(id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        self.frame = CGRectMake(0, CGRectGetMinY(frame), SCREEN_W, CGRectGetHeight(frame));
...
{
    self.frame = CGRectMake(0, CGRectGetMinY(originalFrame), SCREEN_W, CGRectGetHeight(originalFrame));
}
...
{
    if (!_textField) {
        _textField = [[UITextField alloc]initWithFrame:CGRectMake(15, 10, SCREEN_W-60, 30)];
        _textField.backgroundColor = [UIColor whiteColor];
        _textField.font=[UIFont fontWithName:FONTNAME size:12];
